...
# Obt├®n tu connection string desde: Supabase Dashboard ÔåÆ Settings ÔåÆ Database ÔåÆ Connection string
database_url = os.getenv('DATABASE_URL')
if not database_url:
    raise ValueError(
        "DATABASE_URL no est├í configurada. "
...
# Cambi├í estos valores por tu n├║mero real si quer├®s.
...
def agregar_resena(id):
    ...
    if not nombre or not comentario:
        flash('Por favor complet├í tu nombre y comentario.', 'error')
    ...
    flash('┬íGracias por tu rese├▒a!', 'success')
    return redirect(url_for('producto_detalle', id=id))
...
def enviar_emails_checkout(nombre, email_cliente, telefono_cliente, direccion_cliente, cp_cliente, 
    ...
    try:
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="padding:20px 0;">
                ...
                <td align="center">
                    ...
                    <tr>
                        ...
                        <p>┬íGracias por tu compra en <strong>Estilo Fachero</strong>! Estos son los detalles de tu pedido:</p>
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <p style="margin:0 0 8px 0;">Para completar el pago, realiz├í una transferencia por <strong>${total:.2f}</strong> a:</p>
                        <p style="margin:0;">
        ...
        <html>
            ...
            <table width="600" align="center" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
                ...
                <td style="background:#4f5d2f;color:#ffffff;padding:16px 24px;border-bottom:4px solid #4F5D2F;">
                    ...
                    <tr>
                        <td align="left" style="vertical-align:middle;">
                        <h2 style="margin:0;font-size:18px;">┬íNueva venta!</h2>
        ...
        payload_cliente = {
            "to": email_cliente,
            "subject": "┬íGracias por tu compra en Estilo Fachero! Instrucciones de pago",
    ...
    if request.method == 'POST':
        ...
        for item in carrito:
            ...
            if not producto or not producto.activo:
                flash(f'El producto "{item.get("nombre", "")}" no est├í disponible.', 'error')
        ...
        # --- DEDUCIR STOCK (Solo despu├®s de validar que todo est├í OK) ---
        for item in carrito:
            try:
                pid = int(item.get("id"))
                cant_pedida = int(item.get("cantidad", 1))
        ...
        if metodo_pago == 'mercadopago' and sdk:
            ...
            try:
                ...
                if "init_point" not in preference:
                    ...
                    flash("Error al generar el pago. Por favor verific├í tus datos.", "error")
                    return redirect(url_for('cart'))
                    
                return redirect(preference["init_point"])
            except Exception as e:
                print(f"Error EXCEPCI├ôN creando preferencia MP: {e}")
                flash("Hubo un error al conectar con Mercado Pago. Intent├í de nuevo o eleg├¡ transferencia.", "error")
                return redirect(url_for('cart'))
        ...
        # --- ENV├ìO DE MAILS EN SEGUNDO PLANO (THREAD) ---
        email_thread = threading.Thread(
            target=enviar_emails_checkout,
            args=(
                nombre, 
                email_cliente, 
                telefono_cliente, 
...
def admin_login():
    ...
    if request.method == 'POST':
        ...
        if admin and admin.check_password(password):
            login_user(admin)
            flash('┬íBienvenido al panel admin!', 'success')
            return redirect(url_for('admin_panel'))
        else:
...
def admin_panel():
    ...
    # Datos para el gr├ífico (├║ltimos 7 d├¡as)
    ...
    # Ordenar para el gr├ífico
    fechas_grafico = sorted(ventas_por_fecha.keys())[-7:] # ├Ültimos 7 d├¡as con ventas
...
@login_required
def admin_ventas():
    # Obtener par├ímetros de filtrado
    ...
    if filtro_fecha:
        try:
            ...
            pass  # Si el formato es inv├ílido, ignorar el filtro
            ...
            <table width="600" align="center" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
              <tr>
                <td style="background:#4f5d2f;color:#ffffff;padding:16px 24px;border-bottom:4px solid #4F5D2F;">
                  <table width="100%" cellpadding="0" cellspacing="0">
                    <tr>
                      <td align="left" style="vertical-align:middle;">
                        <h2 style="margin:0;font-size:18px;">┬íTu pedido est├í en camino! ­ƒÜÜ</h2>
                      </td>
                      <td align="right" style="vertical-align:middle;">
                        <img src="https://estilo-fachero.onrender.com/static/img/logo.png" alt="Estilo Fachero" style="height:40px;border-radius:50%;">
                      </td>
              ...
              <tr>
                <td style="padding:24px;color:#111827;">
                  ...
                  <p>┬íGracias por elegir Estilo Fachero!</p>
                </td>
              </tr>
            </table>
          </body>
...
def admin_actualizar_venta(id):
    ...
    # Validar que no se pueda volver atr├ís si ya est├í enviado
        ...
        <div class="col-sm-7">
            ...
            <div class="row g-2 small text-muted">
                ...
                <div class="col-12 ps-4">
                    {producto.alto_cm}cm (alto) ├ù {producto.ancho_cm}cm (ancho) ├ù {producto.largo_cm}cm (largo)
                </div>
            </div>
        </div>
    </div>
...
def admin_producto_nuevo():
    if request.method == 'POST':
        ...
        if not nombre or tipo not in TIPOS_PRODUCTO or precio <= 0:
            flash('Completa nombre, tipo (gorra/lentes/medias) y precio v├ílido', 'error')
            return redirect(url_for('admin_producto_nuevo'))
...
def enviar_mail_confirmacion_pago(pedido, payment_id):
    ...
    def _enviar():
        try:
                ...
                <table width="600" align="center" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
                  <tr>
                    <td style="background:#4f5d2f;color:#ffffff;padding:16px 24px;border-bottom:4px solid #4F5D2F;">
                      <table width="100%" cellpadding="0" cellspacing="0">
                        <tr>
                          <td align="left" style="vertical-align:middle;">
                            <h2 style="margin:0;font-size:18px;">┬íPago Confirmado! ­ƒÄë</h2>
                          </td>
                          <td align="right" style="vertical-align:middle;">
                            <img src="https://estilo-fachero.onrender.com/static/img/logo.png" alt="Estilo Fachero" style="height:40px;border-radius:50%;">
                          </td>
                  ...
                  <tr>
                    <td style="padding:24px;color:#111827;">
                      ...
                      <p>Pronto estaremos preparando tu pedido para el env├¡o o retiro.</p>
                      <p>┬íGracias por elegir Estilo Fachero!</p>
                    </td>
                  </tr>
                </table>
            ...
            print(f"Error enviando mail confirmaci├│n pago: {e}", flush=True)
...
if __name__ == '__main__':
    with app.app_context():
        ...
        # --- Migraci├│n autom├ítica de base de datos ---
        try:
            ...
            print(f"Nota: No se pudo actualizar la estructura de la DB autom├íticamente (puede que ya est├® actualizada): {e}")
